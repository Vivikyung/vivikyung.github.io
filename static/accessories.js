...
var initialEntries = function(head, neck, outfit, currdate){
    $.each(head, function(idx, item){
        var row = "<div class='row clothing accessory'><img src='" + item.image + "' id='" + item.name + "' width=180px height=120px></div>"
        $("#head").append(row)
    })
    $.each(neck, function(idx, item){
        var row = "<div class='row clothing accessory'><img src='" + item.image + "' id='" + item.name + "' width=180px height=120px></div>"
        $("#neck").append(row)
    })
...
$(document).ready(function(){
    initialEntries(head, neck, outfit, currdate)
    enableDrag()
    $(".dhead").droppable({
        accept: ".clothing",
        classes: {
        ...
        drop: function(event, ui){
            ...
            move_to_outfit($("#date_header").text(), "accessory", $(image).attr('src'))
        }
    })
