...
var initialEntries = function(outerwear, innerwear, outfit, currdate){
    $.each(outerwear, function(idx, item){
        var row = "<div class='row clothing top'><img src='" + item.image + "' id='" + item.name + "' width=240px height=240px></div>"
        $("#outerwear").append(row)
    })
    $.each(innerwear, function(idx, item){
        var row = "<div class='row clothing top'><img src='" + item.image + "' id='" + item.name + "' width=240px height=240px></div>"
        $("#innerwear").append(row)
    })
...
$(document).ready(function(){
    initialEntries(outerwear, innerwear, outfit, currdate)
    enableDrag()
    $(".dhead").droppable({
        accept: ".clothing",
        classes: {
        ...
        drop: function(event, ui){
            ...
            move_to_outfit($("#date_header").text(), "top", $(image).attr('src'))
        }
    })
