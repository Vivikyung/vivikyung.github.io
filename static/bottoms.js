...
var initialEntries = function(pants, shorts, outfit, currdate){
    $.each(pants, function(idx, item){
        var row = "<div class='row clothing bottom'><img src='" + item.image + "' id='" + item.name + "' width=240px height=240px></div>"
        $("#pants").append(row)
    })
    $.each(shorts, function(idx, item){
        var row = "<div class='row clothing bottom'><img src='" + item.image + "' id='" + item.name + "' width=240px height=240px></div>"
        $("#shorts").append(row)
    })
...
$(document).ready(function(){
    ...
    initialEntries(pants, shorts, outfit, currdate)
    enableDrag()
    $(".dhead").droppable({
        accept: ".clothing",
        classes: {
        ...
        drop: function(event, ui){
            ...
            move_to_outfit($("#date_header").text(), "bottom", $(image).attr('src'))
        }
    })
